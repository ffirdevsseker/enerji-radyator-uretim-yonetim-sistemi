...
```
┌─────────────────┐       ┌──────────────────┐       ┌─────────────────┐
│  musteriler     │       │  tedarikciler    │       │  ham_maddeler   │
├─────────────────┤       ├──────────────────┤       ├─────────────────┤
│ id (PK)         │       │ id (PK)          │       │ id (PK)         │
│ adi             │       │ adi              │       │ adi             │
│ musteri_tipi    │       │ yetkili_kisi     │       │ birim           │
│ vergi_no_tc     │       │ telefon          │       │ depo_stok       │
│ telefon         │       │ adres            │       │ fabrika_stok    │
│ adres           │       └──────────────────┘       │ minimum_stok    │
└─────────────────┘                │                 │ liste_fiyati    │
        │                           │                 └─────────────────┘
        │                           │                          │
        ...
        ▼                           ▼                          ▼
┌─────────────────┐       ┌──────────────────┐       ┌─────────────────┐
│ fatura_kalemleri│       │ satinalma_       │       │ stok_hareketleri│
│                 │       │ hareketleri      │       │                 │
├─────────────────┤       ├──────────────────┤       ├─────────────────┤
│ id (PK)         │       │ id (PK)          │       │ id (PK)         │
│ musteri_id (FK) │       │ tedarikci_id(FK) │       │ hammadde_id(FK) │
│ radyator_id(FK) │       │ hammadde_id (FK) │       │ radyator_id(FK) │
│ miktar          │       │ miktar           │       │ hareket_tipi    │
│ birim_fiyat     │       │ alim_fiyat       │       │ miktar          │
└─────────────────┘       │ fatura_no        │       │ kaynak_tablo    │
                          │ alim_tarihi      │       └─────────────────┘
                          └──────────────────┘
                                   │
                                   │
                                   ▼
┌─────────────────┐       ┌──────────────────┐       ┌─────────────────┐
│  radyotorler    │       │ maliyet_dosyasi  │       │ uretim_         │
│                 │       │                  │       │ irsaliyeleri    │
├─────────────────┤       ├──────────────────┤       ├─────────────────┤
│ id (PK)         │◄──────│ radyator_id (FK) │       │ id (PK)         │
│ adi             │       │ stok_kodu        │       │ irsaliye_no     │
│ kategori        │       │ stok_adi         │       │ tarih           │
│ stok_miktari    │       │ birim            │       │ irsaliye_tipi   │
│ olcu            │       │ miktar           │       │ aciklama        │
│ dilim_sayisi    │       │ fiyat            │       └─────────────────┘
└─────────────────┘       └──────────────────┘                │
        │                                                      │
        │                                                      ▼
        ...
        └─────────────────────────►│ irsaliye_kalemleri           │
                                   ...
                                   │ irsaliye_id (FK)             │
                                   │ urun_tipi                    │
                                   │ hammadde_id (FK) [nullable]  │
                                   │ radyator_id (FK) [nullable]  │
                                   │ miktar                       │
                                   │ birim_fiyat                  │
                                   └──────────────────────────────┘
```
...
1. **musteriler:** Müşteri bilgilerini saklar
2. **tedarikciler:** Tedarikçi bilgilerini saklar
3. **ham_maddeler:** Hammadde stoklarını ve bilgilerini saklar
4. **radyotorler:** Radyatör ürün bilgilerini ve stoklarını saklar
5. **satinalma_hareketleri:** Tedarikçilerden hammadde alımlarını kaydeder
6. **stok_hareketleri:** Tüm stok giriş/çıkış hareketlerini loglar
7. **uretim_irsaliyeleri:** Fabrikaya hammadde çıkışı ve radyatör girişi irsaliyelerini saklar
8. **irsaliye_kalemleri:** İrsaliye kalemlerini detaylı olarak saklar
9. **maliyet_dosyasi:** Her radyatör için gerekli hammaddeleri ve maliyetleri tanımlar
10. **fatura_kalemleri:** Müşteri satış fatura kalemlerini saklar
11. **kalan_hammadde_kanit:** Fabrikada kullanılan hammaddelerin kanıt tablosu
